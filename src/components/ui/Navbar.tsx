...
import { CategoryWithSubcategories } from "@/types/productTypes";
    ...
    <header className="sticky top-0 z-50 bg-[var(--background-color)]/80 backdrop-blur-sm">
            ...
            {dropdownOpen && (
              <div className="absolute top-full left-0 mt-2 bg-[var(--card-background-color)] rounded-xl shadow-lg p-2 min-w-[200px]">
                ...
                {categoriesWithSubcategories.map(cat => (
                  <div key={cat.id}>
                    <Link href={`/productos/${cat.name.toLowerCase()}`} className="block p-2 text-white font-semibold hover:bg-gray-700 rounded transition-colors duration-200">
                      {cat.name}
                    </Link>
                    {cat.subcategories.map(sub => (
                      <Link key={sub.id} href={`/productos/${cat.name.toLowerCase()}/${sub.name.toLowerCase()}`} className="block pl-6 p-2 text-gray-400 hover:bg-gray-700 rounded transition-colors duration-200">
                        {sub.name}
                      </Link>
                    ))}
                  </div>
      ...
      {open && (
        <div className="md:hidden bg-[var(--background-color)] border-t border-gray-700 px-4 py-2 space-y-2">
          ...
          {categoriesWithSubcategories.map(cat => (
            <div key={cat.id}>
              <Link href={`/productos/${cat.name.toLowerCase()}`} className="block text-white py-2 hover:text-gray-400 transition-colors duration-200">{cat.name}</Link>
              {cat.subcategories.map(sub => (
                <Link key={sub.id} href={`/productos/${cat.name.toLowerCase()}/${sub.name.toLowerCase()}`} className="block pl-4 text-gray-400 py-2 hover:text-gray-400 transition-colors duration-200">
                  {sub.name}
                </Link>
              ))}
            </div>
